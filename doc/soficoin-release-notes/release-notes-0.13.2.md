Soficoin Core version 0.13.2 is now available from:

  <https://download.soficoin.org/soficoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/soficoin-project/soficoin/issues>
...
updates will be released anymore. Without security updates, using a soficoin
...
ECDSA signatures inside Soficoin transactions now use validation using
...
Previous versions of Soficoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Soficoin Core was to raise the default minimum
relay fee.

Soficoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Soficoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running soficoind without having to do any manual configuration.
...
Soficoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Soficoin Core will relay transactions with
...
entirely in Soficoin Core 0.13, and it is at this time undecided whether the
...
Soficoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Soficoin Core automatically creates a hidden service to listen on, without
manual configuration. Soficoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Soficoin Core is listening, and
...
Soficoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Soficoin
Core to use floating fees. By default, Soficoin Core will use floating
fees.
...
(default: `0.0002` SFC/kB).

At all times, Soficoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SFC.
Furthermore, Soficoin Core will never create transactions paying less than
...
`soficoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 219819 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 219819 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /soficoinrpc>
        ProxyPass http://127.0.0.1:19819/
        ProxyPassReverse http://127.0.0.1:19819/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass soficoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< soficoinrpc:<password>
    </Location>
...
Restarting soficoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in soficoin.conf
...
soficoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/soficoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Soficoin Core code base has
...
- `soficoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `soficoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Soficoin scripting system that in combination with
...
Soficoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Soficoin Core.
...
The two phases are distinct in the Soficoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Soficoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Soficoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Soficoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of soficoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of soficoins being spent is exactly
...
- **Increased security for multisig:** Soficoin addresses (both P2PKH addresses
  ...
  believed to be provided by Soficoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Soficoin's current ledger (called the UTXO set),
  ...
  note that Soficoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Soficoin users to individually opt-in to almost any change in the Soficoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Soficoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Soficoin
client to Soficoin Core 0.13.2 to signal for segwit support. When a
...
Since Soficoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `soficoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `soficoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
